...
# Also suppress cornac's internal logging
import logging
logging.getLogger('cornac').setLevel(logging.ERROR)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--llm_embeddings_file", type=str, default=None, help="file with LLM embeddings")
    ...
    for key in CF_map:
        CF_best_params_map[key], best_recsys, best_recall = hsearch(
            ...
            tr_data,
            te_data,
            llm_embeddings_file=args.llm_embeddings_file,
            gridsearch=True)
    ...
    for key in CF_map:
        ...
        if key == "wmf" and args.llm_embeddings_file:
            # Use LLM initialization for WMF
            from collaborative_filtering import load_llm_embeddings, WMFWithLLMInit
            llm_embeddings = load_llm_embeddings(args.llm_embeddings_file, J, iid_map)
            if llm_embeddings is not None:
                params_clean = {k: v for k, v in CF_best_params_map[key].items() if k != 'name'}
    ...
    for k in gathered_k_choice_arr:
        ...
        for key in CF_map:
            ...
            print(f"MRS {key} training with k={k} items: {training_time:.2f}s")
