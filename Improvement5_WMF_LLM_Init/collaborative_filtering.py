...
class WMFWithLLMInit(cornac.models.WMF):
    """WMF with LLM embedding initialization and optional freezing."""
    
    def __init__(self, llm_embeddings=None, freeze_iterations=20, **kwargs):
        super().__init__(**kwargs)
        self.llm_embeddings = llm_embeddings
        self.freeze_iterations = freeze_iterations
        self._original_max_iter = None
        
    def fit(self, train_set, val_set=None):
        if self.llm_embeddings is not None:
            # Store original max_iter
            if self._original_max_iter is None:
                self._original_max_iter = self.max_iter
            
            # First, do normal initialization
            super().fit(train_set, val_set)
            
            # Replace item embeddings with LLM embeddings
            llm_emb = self.llm_embeddings.copy()
            llm_emb = llm_emb / np.std(llm_emb) * 0.1 
            if llm_emb.shape[1] != self.k:
                if llm_emb.shape[1] < self.k:
                    # Pad with small random values
                    padding = np.random.normal(0, 0.01, (llm_emb.shape[0], self.k - llm_emb.shape[1]))
                    llm_emb = np.hstack([llm_emb, padding])
                else:
                    # Truncate
                    llm_emb = llm_emb[:, :self.k]
            
            # Initialize item factors
            self.V[:] = llm_emb.astype(np.float32)
            
            # Phase 1: Train only user factors (freeze item factors)
            if self.freeze_iterations > 0:
                frozen_V = self.V.copy()
                self.max_iter = self.freeze_iterations
                
                # Override the item factor update temporarily
                original_V = self.V
                super().fit(train_set, val_set)
                self.V[:] = frozen_V  # Restore frozen embeddings
            
            # Phase 2: Fine-tune both user and item factors
            remaining_iters = self._original_max_iter - self.freeze_iterations
            if remaining_iters > 0:
                self.max_iter = remaining_iters
                super().fit(train_set, val_set)
            
            # Restore original max_iter
            self.max_iter = self._original_max_iter
        else:
            # Normal training without LLM initialization
            super().fit(train_set, val_set)


def load_llm_embeddings(file_path: str, num_items: int, iid_map: dict) -> Optional[np.ndarray]:
    """Load LLM embeddings from JSON file."""
    try:
        ...
        embedding_matrix = np.random.normal(0, 0.01, (num_items, embedding_dim))
        
        # Fill in available embeddings
        for poi_id, embedding in embeddings_dict.items():
            poi_id_int = int(poi_id)
            if poi_id_int in iid_map:
                item_idx = iid_map[poi_id_int]
                embedding_matrix[item_idx] = np.array(embedding)
        
        print(f"Loaded LLM embeddings: {embedding_matrix.shape}, dimension={embedding_dim}")
        return embedding_matrix.astype(np.float32)
    except Exception as e:
        print(f"Failed to load LLM embeddings: {e}")
        return None
...
def hsearch(
    ...
    # Load LLM embeddings if available and model is WMF
    llm_embeddings = None
    if llm_embeddings_file and model.name == "WMF":
        llm_embeddings = load_llm_embeddings(llm_embeddings_file, tr_data.num_items, tr_data.iid_map)
    ...
    while 1:
        ...
        # Create model with LLM initialization if WMF
        if model.name == "WMF" and llm_embeddings is not None:
            params_clean = {k: v for k, v in params.items() if k != 'name'}
            recsys = WMFWithLLMInit(llm_embeddings=llm_embeddings, **params_clean)
            recsys.name = "WMF"
        else:
