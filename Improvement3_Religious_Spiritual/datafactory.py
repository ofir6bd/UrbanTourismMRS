...
def dump_lbsn_artefacts(file: str, y: np.ndarray, f: np.ndarray, fnames: np.ndarray, envstruct: dict, _str: str = "S8"):
    ...
    with h5py.File(file, "w") as o:
        ...
        # NEW: Add religious sites storage
        if "religious_sites" in envstruct:
            o["/environment"].create_dataset("religious_sites", dtype="i8", data=envstruct["religious_sites"])
...
def load_lbsn_artefacts(file: str, _str: str = "U"):
    ...
    with h5py.File(file, "r") as o:
        ...
        # NEW: Load religious sites if available
        if "religious_sites" in g:
            envstruct["religious_sites"] = g["religious_sites"][...].astype("i8")
        else:
            envstruct["religious_sites"] = np.array([], dtype="i8")
