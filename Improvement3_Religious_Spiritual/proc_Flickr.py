...
def identify_religious_sites(tb_item, rank_to_item):
    """Identify religious sites based on categories"""
    religious_keywords = [
        "basilica", "chiesa", "church", "cathedral", "chapel", "monastery", 
        "convent", "abbey", "temple", "mosque", "synagogue", "shrine",
        "santo", "santa", "san", "saint", "madonna", "cristo", "jesus"
    ]
    
    religious_sites = []
    for rank in range(len(rank_to_item)):
        item_id = rank_to_item[rank]
        # Get POI info
        poi_info = tb_item.filter(pl.col("clus") == item_id)
        if len(poi_info) > 0:
            categories = poi_info["categories"].item().lower() if poi_info["categories"].item() else ""
            pois_name = poi_info["pois"].item().lower() if poi_info["pois"].item() else ""
            
            # Check if any religious keyword is in categories or name
            if any(keyword in categories or keyword in pois_name for keyword in religious_keywords):
                religious_sites.append(rank)
    
    return np.array(religious_sites, dtype="i8")
...
if __name__ == "__main__":
    ...
    for city in city_arr:
        ...
        new_rank_to_item = {}
        for new_rank, old_rank in enumerate(pop_ranking_ix):
            if old_rank in rank_to_item:
                new_rank_to_item[new_rank] = rank_to_item[old_rank]

        # NEW: Identify religious sites after all filtering
        religious_sites = identify_religious_sites(tb_item, new_rank_to_item)
