...
from utils import argnonz, haversine


def calculate_total_distance(choices, locations):
    """Calculate total travel distance for a set of POI choices"""
    if len(choices) <= 1:
        return 0.0
    
    total_distance = 0.0
    for i in range(len(choices) - 1):
        total_distance += haversine(locations[choices[i]], locations[choices[i+1]])
    return total_distance


def optimize_route_nearest_neighbor(choices, locations):
    """
    Optimize route using nearest neighbor starting from first POI.
    Returns POIs reordered for shorter total distance.
    """
    if len(choices) <= 1:
        return choices
    
    # Start with first picked POI
    route = [choices[0]]
    remaining = list(choices[1:])
    
    current_poi = choices[0]
    
    # Find nearest unvisited POI each time
    while remaining:
        current_location = locations[current_poi]
        
        # Find nearest POI from remaining ones
        min_distance = float('inf')
        nearest_poi = None
        
        for poi in remaining:
            distance = haversine(current_location, locations[poi])
            if distance < min_distance:
                min_distance = distance
                nearest_poi = poi
        
        # Add nearest POI to route and remove from remaining
        route.append(nearest_poi)
        remaining.remove(nearest_poi)
        current_poi = nearest_poi
    
    return np.array(route, dtype="i8")
...
def simulate_1_day(
    ...
    croi = args["croi"]
    distance_penalty_weight = args.get("distance_penalty_weight", 0.1)  # default penalty weight

    # -------------------------------------------------------------------------

    # load dataset with lsbn trajectories and environment structure
    y_lbsn, _, _, envstruct = load_lbsn_artefacts(lbsn_file)
    poi_locations = envstruct["location"]
    ...
    for t, act in users_arrival_generator(rate=rate, seed=seed):
        ...
        # Get user's starting location (use city centre as default, or could use first known POI)
        user_known_pois = argnonz(A[n,:])
        if len(user_known_pois) > 0:
            # Start from a random known POI location
            start_poi = np.random.choice(user_known_pois)
            user_location = poi_locations[start_poi,:]
        else:
            # Fallback to city centre
            user_location = city_centre

        # ---------------------------------------------------------------------
        # RECOMMENDER SYSTEM FORESEES WHAT PACK TO RECOMMEND (AS A PROPOSITION)

        void_pack = recommend_with_void_search_decoder(
            pred_user_preferences[n,:], 
            advt_preferences, 
            poi_locations,
            user_location,
            pack_size, 
        ...
        cond_pack = recommend_with_void_search_decoder(
            pred_user_preferences[n,:], 
            advt_preferences, 
            poi_locations,
            user_location,
        ...
        accepted_void_pack = make_boltzman_choices(true_user_preferences[n,:] + boost, void_pack, anset,
                                                   need_size, beta[n])
        # OPTIMIZE ROUTE FOR VOID PACK
        accepted_void_pack = optimize_route_nearest_neighbor(accepted_void_pack, poi_locations)
        ...
        accepted_cond_pack = make_boltzman_choices(true_user_preferences[n,:] + boost, cond_pack, anset,
                                                   need_size, beta[n])
        # OPTIMIZE ROUTE FOR COND PACK
        accepted_cond_pack = optimize_route_nearest_neighbor(accepted_cond_pack, poi_locations)

        boost = 0
        accepted_nors_pack = make_boltzman_choices(true_user_preferences[n,:] + boost, nors_pack, anset,
                                                   need_size, beta[n])
        # OPTIMIZE ROUTE FOR NORS PACK
        accepted_nors_pack = optimize_route_nearest_neighbor(accepted_nors_pack, poi_locations)

        accepted_data_pack = argnonz(y_true[n,:])
        # OPTIMIZE ROUTE FOR DATA PACK
        accepted_data_pack = optimize_route_nearest_neighbor(accepted_data_pack, poi_locations)
        ...
        # -----------------------------
        # Calculate and record distances (using existing function with reordered POIs)

        void_distance = calculate_total_distance(accepted_void_pack, poi_locations)
        cond_distance = calculate_total_distance(accepted_cond_pack, poi_locations)
        nors_distance = calculate_total_distance(accepted_nors_pack, poi_locations)
        data_distance = calculate_total_distance(accepted_data_pack, poi_locations)

        log["void_total_distance"].append(void_distance)
        log["cond_total_distance"].append(cond_distance)
        log["nors_total_distance"].append(nors_distance)
        log["data_total_distance"].append(data_distance)
        ...
        if disp_ok:
            print(
                ...
                f"{nors_harm:>6.0f}  |  "
                f"dist= "
                f"{void_distance:>5.1f}  "
                f"{cond_distance:>5.1f}  "
                f"{nors_distance:>5.1f}  "
            )
...
if __name__ == "__main__":
    ...
    simulate_1_day(
        ...
        args={
            ...
            "croi": croi,
            "distance_penalty_weight": 0.1,  # Add distance penalty parameter
        }
    )
