...
def run_experiment(  # --> high level configuration for `simulate_1_day` experiment simulation
    ...
    experiment: dict[str,dict|list] = {
        ...
        "nors_distances": [],
        "data_distances": [],
        "distance_improvement": [],  # distance reduction (more is better)
        "distance_bias": [],  # distance bias (0 is better)
        "hist": []}
    ...
    for lamb in args["lamb_arr"]:
        ...
        # distance metrics
        void_distances = hist["void_total_distance"]
        nors_distances = hist["nors_total_distance"]
        data_distances = hist["data_total_distance"]
        
        # distance improvement (reduction in travel distance)
        mean_void_distance = np.mean(void_distances)
        mean_nors_distance = np.mean(nors_distances)
        mean_data_distance = np.mean(data_distances)
        
        distance_improvement = (mean_nors_distance - mean_void_distance) / mean_nors_distance if mean_nors_distance > 0 else 0
        distance_bias = (mean_nors_distance - mean_data_distance) / mean_data_distance if mean_data_distance > 0 else 0
        ...
        experiment["void_distances"].append(void_distances)
        experiment["nors_distances"].append(nors_distances)
        experiment["data_distances"].append(data_distances)
        experiment["distance_improvement"].append(distance_improvement)
        experiment["distance_bias"].append(distance_bias)
        experiment["hist"].append({
            __k: hist[__k] for __k in ("void_choice_arr",
                                       "nors_choice_arr",
                                       "data_choice_arr", "void_exposure_arr",
                                       "void_total_distance", "nors_total_distance", "data_total_distance")
        })
        ...
        print(f"---------------------------------\n"
              ...
              f"dist lift={distance_improvement:.4f} | "
              f"user bias={bias_improvement_user:.4f} and sust bias={bias_improvement_harm:.4f} and dist bias={distance_bias:.4f} > "
              f"\n\n")
