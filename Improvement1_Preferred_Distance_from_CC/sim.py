...
from utils import argnonz, rand_choice_arr_nb
...
def simulate_1_day(
    ...
    y, _, _, envstruct = load_lbsn_artefacts(lbsn_file)
    city_centre = envstruct["city_centre"]
    city_radius = envstruct["city_radius"]
    location = envstruct["location"]
    ...
    # -------------------------------------------------------------------------
    # RECREATE THE SAME t ITEMS USED DURING TRAINING
    
    # Get the t parameter (number of known items per user)
    t = int(recommender_system_t_key)
    
    # Calculate distances from city center for all POIs
    poi_distances = np.array([haversine(city_centre, location[j]) for j in range(J)])
    
    # Recreate the same sampling process used in run1.py
    # This uses the same SEED (2024) and same sampling method
    TRAINING_SEED = 2024  # Same seed used in run1.py
    np.random.seed(TRAINING_SEED)  # Set seed to match training
    
    # Get popularity distribution (same as in run1.py)
    distribution_pop = y_true.sum(axis=0) / y_true.sum()
    
    # Initialize user distance preferences
    user_distance_pref = np.zeros(N, dtype="f8")
    user_sampled_items = {}  # Store the sampled items for each user
    distance_weight = 0.2  # weight for distance penalty
    
    for n in range(N):
        # Recreate the exact same sampling process from run1.py
        hot_arr = argnonz(y_true[n,:])  # index of true choices
        hot_len = hot_arr.size
        
        if hot_len > 0:
            # Use the EXACT same sampling method as in run1.py
            # This samples based on popularity bias
            sampled_indices = rand_choice_arr_nb(
                p=distribution_pop[hot_arr] / distribution_pop[hot_arr].sum(), 
                size=min(t, hot_len)
            )
            sampled_items = hot_arr[sampled_indices]
            
            # Store the sampled items for this user
            user_sampled_items[n] = sampled_items
            
            # Calculate average distance of user's t sampled items
            user_distance_pref[n] = np.mean(poi_distances[sampled_items])
        else:
            # Fallback: use city center area
            user_sampled_items[n] = np.array([], dtype="i8")
            user_distance_pref[n] = 0.3 * city_radius

    # Reset random seed for the simulation
    np.random.seed(seed)
    ...
    log = {
        ...
        "user_sampled_items": user_sampled_items,  # Add this to log for verification
    }

    # -------------------------------------------------------------------------

    for t_sim, act in users_arrival_generator(rate=rate, seed=seed):
        ...
        # ---------------------------------------------------------------------
        # DURING RECOMMENDATION: Apply individual distance penalty for current user
        
        # Calculate distance penalty for each POI based on THIS user's preferred distance
        distance_penalty = np.abs(poi_distances - user_distance_pref[n])
        
        # Apply distance penalty to user preferences before calling recommender
        adjusted_user_pref = pred_user_preferences[n,:] - distance_weight * distance_penalty

        # ---------------------------------------------------------------------
        # RECOMMENDER SYSTEM FORESEES WHAT PACK TO RECOMMEND (AS A PROPOSITION)

        void_pack = recommend_with_void_search_decoder(
            adjusted_user_pref, advt_preferences, pack_size, lamb)
        ...
        if disp_ok:
            sampled_items_str = str(user_sampled_items[n][:3]) if len(user_sampled_items[n]) > 0 else "[]"
            print(
                f"{len(log['n']):>05d} | {n:>04d} | {toc-tic:>6.4f} sec  k={need_size:>2d}"
                f"  pref_dist={user_distance_pref[n]:>5.2f}km  sampled={sampled_items_str}..."
                f"  util= "
                f"{void_user:>5.2f}  "
                f"{cond_user:>5.2f}  "
                f"{nors_user:>5.2f}  |  "
